...
@router.post("/applicants", response_model=ApplicantSchema, status_code=status.HTTP_201_CREATED)
def create_applicant(applicant: ApplicantCreate, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        # Check if the error is specifically due to the unique email constraint
        # This check might need to be more robust depending on the database used
        if "email_address" in str(e):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Applicant with this email address already exists")
        else:
